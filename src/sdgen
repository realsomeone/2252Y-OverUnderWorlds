...
wait(1,SECONDS)
player.rumble('..')
player.screen.print('go')
active = Thread(driver.broadcast)
brain.timer.reset()
while True:
    if player.buttonUp.pressing():
        break
    data += "%5.3f" % lefty.velocity(PERCENT) + ','
    data += "%5.3f" % right.velocity(PERCENT) + ','
    data += "%5.3f" % intake.velocity(PERCENT) + ','
    data += "%5.3f" % fwing.value() + ','
    data += "%5.3f" % lbwing.value() + '\n'
    wait(polling)
brain.sdcard.savefile(fname,bytearray(data,'utf-8'))
active.stop()
player.screen.clear_line
player.screen.print('saved')